---
title: "compstatsR:<br>C++ and Rcpp"
author: "Ben Bolker"
date: "19 March 2026"
...
- check algorithms  
(FitzJohn: "Using C will not magically make a bad algorithm faster")
...
- older
- [much more widely used](https://www.tiobe.com/tiobe-index/)  
(in 2026: #3 after Python and C, R is #9)
...
## example (@wickham_advanced_2019) {.columns-2}

```{r rcpp1, message=FALSE}
library(Rcpp)
cppFunction(
...
## Style note: early returns [@bryanCode2018] {.columns-2}

```{r switch1, eval=FALSE}
if (x>0) {
  1
} else if (x==0) {
  0
...
- (source-level debugging of C++-in-R is [clunky](https://dirk.eddelbuettel.com/papers/rcpp_uzuerich_2015_part5_packaging.pdf)); see also [here](https://blog.davisvaughan.com/posts/2019-04-05-debug-r-package-with-cpp/), [here](https://contributions.bioconductor.org/debugging-cc-code.html), or [here](https://blog.davisvaughan.com/posts/2022-03-11-using-vs-code-to-debug-r-packages-with-c-code/)
...
(via [Dirk Eddelbuettel](https://dirk.eddelbuettel.com/blog/2011/07/14/#rcpp_gibbs_example), + some ChatGPT)
...
```{r gibbs_run}
system.time(gibbs_res <- gibbs(n = 5e4*10, thin = 10))
```

$\approx$ 24 times faster than pure R ([code](https://github.com/bbolker/compstatsR/blob/master/gibbs_bench.R))

## plot

```{r gibbsplot, cache = TRUE}
ggplot(gibbs_res, aes(x,y)) +
  ggrastr::rasterise(geom_point(pch = ".", alpha = 0.4)) +
...
- (**but** note that vectorization here is for readability, not speed)
...
- eigenvalues/vectors; decompositions (Cholesky, QR, SVD, etc.); sparse matrices; ...
...
https://bbolker.github.io/compstatsR

```{r qrcode, echo = FALSE}
library("qrcode")
plot(qr_code("https://github.com/bbolker/compstatsR"))
```
